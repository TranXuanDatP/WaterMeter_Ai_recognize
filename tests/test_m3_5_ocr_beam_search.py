"""
Test OCR Finetune Model + Beam Search on M3.5 Black Digits Output

Test M3.5 pipeline output (black digit fragments) with OCR model and beam search decoder.
This is the CORRECT test - M3.5 contains pre-extracted black digit fragments.
"""
import os
import sys
import cv2
...
# M3.5 pipeline output directory (BLACK DIGIT FRAGMENTS)
INPUT_DIR = Path(r"F:\Workspace\Project\results\pipeline_full_m1_m2_m3_m3_5_m4_beam_backup_20260309_155359\m3_5_black_digits")

# OCR finetuned model (117MB)
MODEL_PATH = Path(r"F:\Workspace\Project\model\ocr_finetune.pth")

# Output directory
OUTPUT_DIR = Path(r"F:\Workspace\Project\results\test_m3_5_ocr_beam_search")
...
print("=" * 80)
print("TEST OCR FINETUNE + BEAM SEARCH ON M3.5 BLACK DIGITS")
print("=" * 80)
print(f"\nInput: {INPUT_DIR}")
...
def test_ocr_beam_search(model, decoder, input_dir, output_dir, device):
    """Test OCR with beam search on M3.5 output images"""
    ...
    if len(results) > 0:
        ...
        for count, num_files in digit_counts.items():
            print(f"  {count} digits: {num_files} files ({num_files/len(results)*100:.1f}%)")
