"""
M2 Orientation + Smart Rotate on data_4digit2 Dataset

Applies M2 orientation prediction and smart rotation to all images
in the data_4digit2 dataset and saves the results.
"""
import sys
import cv2
import torch
...
class Config:
    ...
    OUTPUT_DIR = Path(r"F:\Workspace\Project\results\m2_rotated_data_4digit2")
...
class M2_SmartRotator:
    ...
    def smart_rotate(self, image: np.ndarray, angle: float) -> tuple:
        ...
        Returns:
            (rotated_image, actual_angle_used)
        """
...
def main():
    print("=" * 80)
    print("M2 ORIENTATION + SMART ROTATE ON DATA_4DIGIT2")
    print("=" * 80)
    ...
    logger.log("=" * 80)
    logger.log("M2 ORIENTATION + SMART ROTATE ON DATA_4DIGIT2")
    logger.log("=" * 80)
    ...
    for img_path in tqdm(image_files, desc="Rotating"):
        try:
            ...
            # Smart rotate
            rotated, actual_angle = rotator.smart_rotate(img, angle)
            ...
            result = {
                'filename': img_name,
                'predicted_angle': round(angle, 2),
                'actual_rotation': round(actual_angle, 2),
            ...
            logger.stats['angles'].append(actual_angle)
